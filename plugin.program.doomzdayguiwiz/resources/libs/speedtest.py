...
try:
    ...
    class _Py3Utf8Output(TextIOWrapper):
        ...
        def __init__(self, f, **kwargs):
            # Only use FileIO if fileno() exists (not in Kodi)
            if hasattr(f, 'fileno'):
                try:
                    ...
                    # If anything fails, fallback to using f directly
                    super(_Py3Utf8Output, self).__init__(
                        f,
                        encoding='utf8',
                        errors='strict'
                ...
                # Fallback for Kodi: just use f directly
                super(_Py3Utf8Output, self).__init__(
                    f,
                    encoding='utf8',
                    errors='strict'
    ...
    try:
        # Only use _Py3Utf8Output if fileno() exists
...
def speedtest():
    """
    Run a speed test and return a list with the share image URL as the first element.
    This is a wrapper for use in the wizard, with Kodi dialog integration.
    """
    dp = None
    try:
        # Try to import Kodi dialog and uservar for colors/titles
        try:
            import xbmcgui
            import uservar
            ...
            ADDONTITLE = "Speed Test"
            COLOR1 = "white"
            COLOR2 = "yellow"

        # Show Kodi progress dialog if available
        try:
            dp = xbmcgui.DialogProgress()
            dp.create('%s: [COLOR %s]Speed Test[/COLOR]' % (ADDONTITLE, COLOR1), '[COLOR %s]Starting test...[/COLOR]' % COLOR2)
            dp.update(5, '[COLOR %s]Retrieving speedtest.net configuration...[/COLOR]' % COLOR2)
        except Exception:
            dp = None

        st = Speedtest()
        if dp: dp.update(15, '[COLOR %s]Selecting best server...[/COLOR]' % COLOR2)
        st.get_best_server()
        if dp: dp.update(35, '[COLOR %s]Testing download speed...[/COLOR]' % COLOR2)
        st.download()
        if dp: dp.update(65, '[COLOR %s]Testing upload speed...[/COLOR]' % COLOR2)
        st.upload()
        if dp: dp.update(85, '[COLOR %s]Getting share image...[/COLOR]' % COLOR2)
        share_url = st.results.share()
        if dp: dp.update(100, '[COLOR %s]Test complete![/COLOR]' % COLOR2)
        if dp: dp.close()
        return [share_url]
