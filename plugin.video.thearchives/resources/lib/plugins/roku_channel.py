...
class RokuChannel(Plugin):
    ...
    def get_list(self, url):
        ...
        if url == self.home_url:
            try:
                return self.session.get(HOME_API, headers=self._headers()).text
            except Exception:
                return None
        ...
        if url.startswith(self.page_url + "/"):
            ...
            try:
                return self.session.get(api_url, headers=self._headers()).text
            except Exception:
                return None
        ...
        if url == self.search_url:
            ...
            try:
                ...
                return json.dumps({"_query": query, "_results": resp.json()})
            except Exception:
    ...
    def parse_list(self, url: str, response: str) -> Optional[List[Dict[str, str]]]:
        ...
        if (
            ...
            or url == self.search_url
        ):
            self._add_page_items(itemlist, data, collection_title=collection_title)
            if not itemlist:
                itemlist.append({
                    "type": "dir",
    ...
    def _add_page_items(self, itemlist, data, collection_title=""):
        if collection_title:
            for content in _named_collection_contents(data, collection_title):
                self._add_content_item(itemlist, content)
            return
        ...
        for collection in _listish(data.get("collections")):
            ...
            for entry in view:
                self._add_content_item(itemlist, _dictish(entry.get("content")))
    ...
    def _add_content_item(self, itemlist, item, series_thumb="", force_playable=False):
