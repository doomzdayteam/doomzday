...
class _DB:
    ...
    def set(self, url: str, response: str) -> None:
        ...
        if cached:
            ...
            try:
                cached_json = json.loads(c_resp)
                cache_time = cached_json.get("cache_time", self.cache_timer) if isinstance(cached_json, dict) else self.cache_timer
                if (c_created + cache_time*60) > created:
                    created = c_created
...
""",
    ...
    def clear_cache(self) -> None:
        ...
        try:
            from resources.lib.vod_cache import VOD_CACHE
        except ImportError:
            try:
                from .vod_cache import VOD_CACHE
            except ImportError:
                VOD_CACHE = None
        dialog = Dialog()
        if VOD_CACHE:
            VOD_CACHE.clear()
        if not xbmcaddon.Addon().getSettingBool("use_cache"):       
            dialog.ok("Clear Cache", "Cache setting is disabled.\nVOD cache cleared if present.")
            return
